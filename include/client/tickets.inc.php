...
<form action="tickets.php" method="get" id="ticketSearchForm">
    ...
    <select name="status">
        ...
        <option value="open"
            <?php echo ($status=='open')?'selected="selected"':'';?>>Aberto (<?php echo $thisclient->getNumOpenTickets(); ?>)</option>
        <?php
        if($thisclient->getNumClosedTickets()) {
            ?>
        <option value="closed"
            <?php echo ($status=='closed')?'selected="selected"':'';?>>Fechado (<?php echo $thisclient->getNumClosedTickets(); ?>)</option>
        <?php
        } ?>
    </select>
    <input type="submit" value="Go">
</form>
<a class="refresh" href="<?php echo Format::htmlchars($_SERVER['REQUEST_URI']); ?>">Atualizado</a>
<table id="ticketTable" width="800" border="0" cellspacing="0" cellpadding="0">
    ...
    <thead>
        <tr>
            ...
            <th width="100">
                <a href="tickets.php?sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Date">Data de Criação</a>
            </th>
            <th width="80">
                <a href="tickets.php?sort=status&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Status">Status</a>
            </th>
            <th width="300">
                <a href="tickets.php?sort=subj&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Subject">Assunto</a>
            </th>
            <th width="150">
                <a href="tickets.php?sort=dept&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Department">Departamento</a>
            </th>
            <th width="100">Number de Telefone</th>
        </tr>
    </thead>
    <tbody>
    <?php
