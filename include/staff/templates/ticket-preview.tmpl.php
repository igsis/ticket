...
elseif($ticket->isOverdue())
    $warn.='&nbsp;<span class="Icon overdueTicket">Marcado atrasado!</span>';
...
if($ticket->isOpen()) {
    if($ticket->isOverdue())
        $ticket_state.=' &mdash; <span>Atrasado</span>';
    else
...
<div class="tab_content" id="collab" style="display:none;">
    <table border="0" cellspacing="" cellpadding="1">
        ...
        <?php
            ...
            echo "Ticket não tem colaboradores.";
        }?>
    </table>
    <br>
    <?php
    echo sprintf('<span><a class="collaborators"
                            ...
                            $ticket->getId(),
                            $ticket->getNumCollaborators()
                                ? 'Gerenciar Colaboradores' : 'Adicionar Colaboradores'
                                );
    ?>
</div>
<?php
