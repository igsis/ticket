...
<div id="user-profile" style="display:<?php echo $forms ? 'none' : 'block'; ?>;margin:5px;">
    ...
    <a class="action-button pull-right change-user" style="overflow:inherit"
        href="#tickets/<?php echo $ticket->getId(); ?>/change-user" ><i class="icon-user"></i> Alterar Usuário</a>
    <?php
    } ?>
    <div><b><?php
...
<ul class="tabs" style="margin-top:5px">
    <li><a href="#info-tab" class="active"
        ><i class="icon-info-sign"></i>&nbsp;Usuário</a></li>
<?php if ($org) { ?>
    <li><a href="#organization-tab"
        ><i class="icon-fixed-width icon-building"></i>&nbsp;Organização</a></li>
<?php }
    $ext_id = "U".$user->getId();
    $notes = QuickNote::forUser($user, $org)->all(); ?>
    <li><a href="#notes-tab"
        ><i class="icon-fixed-width icon-pushpin"></i>&nbsp;Notas</a></li>
</ul>
...
<div class="body">
    <a href="#"><i class="icon-plus icon-large"></i> &nbsp; Clique para criar uma nova nota</a>
</div>
</div>
</div>
</div>
...
<div><p id="msg_info"><i class="icon-info-sign"></i>&nbsp; Por favor, note que as atualizações serão refletidas em todo o sistema.</p></div>
<?php
...
<form method="post" class="user" action="<?php echo $action; ?>">
    ...
    echo ($ticket && $user) ? 'cancel' : 'close' ?>"  value="Cancelar">
        </span>
