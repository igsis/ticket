...
if (!$info['title'])
    $info['title'] = 'Excluir Usuário: '.Format::htmlchars($user->getName());

$info['warn'] = 'Os usuários excluídos e os bilhetes não podem ser recuperados';
...
<div id="user-profile" style="margin:5px;">
    ...
    <a class="action-button pull-right change-user" style="overflow:inherit"
        ...
        class="icon-user"></i> Alterar Proprietário do Ticket</a>
    <?php
    } ?>
...
?>
    ...
    if (($num=$user->tickets->count())) {
        echo sprintf('<div><input type="checkbox" name="deletetickets" value="1" >
            <strong>Excluir <a href="tickets.php?a=search&uid=%d" target="_blank">%d
            %s</a> e quaisquer anexos e dados associados.</strong></div><hr>',
            $user->getId(),
            $num,
