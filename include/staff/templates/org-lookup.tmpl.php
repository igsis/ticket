...
if (!$info['title'])
    $info['title'] = 'Procurar Organização';

$msg_info = 'Pesquisar organizações existentes ou adicionar uma nova.';
if ($info['search'] === false)
    $msg_info = 'Preencha o formulário abaixo para adicionar uma nova organização.';
...
<form method="post" class="org" action="<?php echo $info['action'] ?: '#orgs/lookup'; ?>">
    ...
    <a class="action-button pull-right" style="overflow:inherit"
        id="unselect-org"  href="#"><i class="icon-remove"></i> Adicionar Nova Organização</a>
...
<form method="post" class="org" action="<?php echo $info['action'] ?: '#orgs/add'; ?>">
    ...
    <?php
        ...
        $form->render(true, 'Criar Nova Organização'); ?>
    </table>
    <hr>
    <p class="full-width">
        <span class="buttons" style="float:left">
            <input type="reset" value="Redefinir">
            <input type="button" name="cancel" class="<?php echo $org ? 'cancel' : 'close' ?>"  value="Cancelar">
        </span>
        <span class="buttons" style="float:right">
