...
<div style="width:700px; float:left;">
   <?php
    if ($results) {
        echo  sprintf('<strong>Mostrando 1 - %de %s</strong>', count($results), count($results));
    } else {
        echo sprintf('%s não tem nenhum ticket', $user? 'Usuário' : 'Organização');
    }
   ?>
</div>
<div style="float:right;text-align:right;padding-right:5px;">
    <?php
    if ($user) { ?>
    <b><a class="Icon newTicket" href="tickets.php?a=open&uid=<?php echo $user->getId(); ?>"> Criar Novo Ticket</a></b>
    <?php
    } ?>
</div>
<br/>
    ...
    foreach($results as $row) {
        ...
        <tr id="<?php echo $row['ticket_id']; ?>">
            ...
            <td align="center" nowrap>
              <a class="Icon <?php echo strtolower($row['source']); ?>Ticket ticketPreview" title="Visualização de Ticket"
