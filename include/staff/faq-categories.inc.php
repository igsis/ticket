...
?>
<h2>Perguntas Frequentes&nbsp;<i class="help-tip icon-question-sign" href="#page_header"></i></h2>
<form id="kbSearch" action="kb.php" method="get">
    ...
    <div>
        ...
        <select name="cid" id="cid">
            <option value="">&mdash; Todas Categorias Categories &mdash;</option>
            <?php
    ...
    <div>
        <select name="topicId" style="width:350px;" id="topic-id">
            <option value="">&mdash; Todos os Tópicos de Ajuda &mdash;</option>
            <?php
...
if($_REQUEST['q'] || $_REQUEST['cid'] || $_REQUEST['topicId']) { //Search.
    ...
    echo "<div><strong>Resultados da Pesquisa</strong></div><div class='clear'></div>";
    if(($res=db_query($sql)) && db_num_rows($res)) {
        echo '<div id="faq">
                <ol>';
        while($row=db_fetch_array($res)) {
        ...
        echo '<strong class="faded">Nenhuma FAQs encontrada na busca.</strong>';
    }
    ...
    $sql='SELECT cat.category_id, cat.name, cat.description, cat.ispublic, count(faq.faq_id) as faqs '
        ...
        .' ORDER BY cat.name';
    if(($res=db_query($sql)) && db_num_rows($res)) {
        echo '<div>Clique em uma categoria para adicionar novas FAQs ou gerencia suas FAQs existentes.&nbsp;</div>
                <ul id="kb">';
        while($row=db_fetch_array($res)) {
            ...
            echo sprintf('
                ...
                ($row['ispublic']?'Público':'Interno'),
                Format::safe_html($row['description']));
        }
        echo '</ul>';
    } else {
        echo 'Nenhuma FAQs encontrada';
    }
}
?>
</div>
