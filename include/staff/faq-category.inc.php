...
<div>
    ...
    <span>(<?php echo $category->isPublic()?'Público':'Interno'; ?>)</span>
    <time>Última Atualização <?php echo Format::db_date($category->getUpdateDate()); ?></time>
</div>
<div class="cat-desc">
...
if($thisstaff->canManageFAQ()) {
    echo sprintf('<div class="cat-manage-bar"><a href="categories.php?id=%d" class="Icon editCategory">Edit Category</a>
             <a href="categories.php" class="Icon deleteCategory">Excluir Categoria</a>
             <a href="faq.php?cid=%d&a=add" class="Icon newFAQ">Adicionar FAQ</a></div>',
            $category->getId(),
            $category->getId());
} else {
?>
...
if(($res=db_query($sql)) && db_num_rows($res)) {
    ...
    echo '<strong>Categoria de FAQs não encontrada</strong>';
}
?>
